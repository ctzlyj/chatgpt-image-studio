...
def _turnstile_to_str(value: Any) -> str:
    ...
    if isinstance(value, str):
        special = {'window.Math': '[object Math]', 'window.Reflect': '[object Reflect]', 'window.performance': '[object Performance]', 'window.localStorage': '[object Storage]', 'window.Object': 'function Object() { [native code] }', 'window.Reflect.set': 'function set() { [native code] }', 'window.performance.now': 'function () { [native code] }', 'window.Object.create': 'function create() { [native code] }', 'window.Object.keys': 'function keys() { [native code] }', 'window.Math.random': 'function random() { [native code] }'}
...
def solve_turnstile_token(dx: str, p: str) -> Optional[str]:
    ...
    def func_17(e: float, t: float, *args: float) -> None:
        ...
        if target == 'window.performance.now':
            elapsed_ns = time.time_ns() - int(start_time * 1000000000.0)
            process_map[e] = (elapsed_ns + random.random()) / 1000000.0
            ...
            if call_args and call_args[0] == 'window.localStorage':
                process_map[e] = ['STATSIG_LOCAL_STORAGE_INTERNAL_STORE_V4', 'STATSIG_LOCAL_STORAGE_STABLE_ID', 'client-correlated-secret', 'oai/apps/capExpiresAt', 'oai-did', 'STATSIG_LOCAL_STORAGE_LOGGING_REQUEST', 'UiState.isNavigationCollapsed.1']
    ...
    process_map.update({1: func_1, 2: func_2, 3: func_3, 5: func_5, 6: func_6, 7: func_7, 8: func_8, 9: token_list, 10: 'window', 14: func_14, 15: func_15, 16: p, 17: func_17, 18: func_18, 19: func_19, 20: func_20, 21: func_21, 23: func_23, 24: func_24})
