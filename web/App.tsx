...
function SettingsDialog({ settings, close, saved }: { settings: Settings; close: () => void; saved: (settings: Settings) => void }) {
  ...
  return <div className="overlay"><section className="dialog settings-dialog" role="dialog" aria-modal="true" aria-label="连接设置"><div className="dialog-heading"><div><h2>连接你的 ChatGPT</h2><p>只连接自己的账号，凭证不进入浏览器历史或代码仓库。</p></div><button aria-label="关闭连接设置" onClick={close}><X/></button></div>
    ...
    <label className="settings-label">网页登录凭证<input type="password" autoComplete="new-password" value={token} onChange={event => setToken(event.target.value)} placeholder={settings.configured ? '已保存；留空不修改' : '只粘贴 accessToken 的值'}/></label>
    <details className="help"><summary>在哪里找到网页登录凭证？</summary><p>在你自己的浏览器登录 ChatGPT，再打开会话信息页，只复制 <code>accessToken</code> 字段的值到此处。不要复制整个 JSON，不要把凭证发送给别人。</p><a href="https://chatgpt.com/api/auth/session" target="_blank" rel="noreferrer">打开 ChatGPT 会话信息页 <ArrowUpRight size={14}/></a><p>凭证有有效期。若遇到验证或无权访问，请先在网页完成登录；本程序不自动注册或绕过权限。</p></details>
